...
export default function UploadPage() {
  ...
  const uploadFile = async (item) => {
    ...
    try {
      ...
      setExtractedTasks(prev => [...prev, taskData]);

      fetchDocs();

      // ── Send to shared context and navigate to Review Tasks ──
  ...
  const totalTasksExtracted = extractedTasks.length + docs.filter(d => d.status === 'Processed').length;
    ...
    <div className="up-page">
      ...
      <div className="up-info-bar">
        ...
        <div className="up-info-card">
          <span className="up-info-icon">✅</span>
          <div>
            <div className="up-info-num">{totalTasksExtracted}</div>
        ...
        <div className="up-left">
              ...
              {files.map(f => (
                <div key={f.id} className="up-queue-item">
                  ...
                  <div className="up-queue-info">
                    ...
                    {!f.error && (
                      <>
                        ...
                        <div className="up-prog-label">
                          {f.done
                            ? '✅ Task extracted! Redirecting to review…'
                    ...
                    {f.done && f.task && (
                      <div className="up-task-preview">
                        <div className="up-task-preview-title">📝 {f.task.task_title}</div>
                        <div className="up-task-preview-meta">
                          <span>📚 {f.task.subject}</span>
                          <span>📅 {f.task.deadline}</span>
                          <span
                            className="up-task-preview-priority"
                            style={priorityColor(f.task.ai_analysis?.recommended_priority)}
                          >
                            {f.task.ai_analysis?.recommended_priority} Priority
                          </span>
                        </div>
                      </div>
                    )}
          ...
          {extractedTasks.length > 0 && (
            <div className="panel" style={{ marginTop: 16 }}>
              <div className="panel-header">
                <div className="panel-title">🎯 Extracted Tasks</div>
                <span className="up-queue-count">{extractedTasks.length} task{extractedTasks.length > 1 ? 's' : ''}</span>
              </div>
              {extractedTasks.map((t, i) => (
                <div key={i} className="up-doc-row">
                  <span className="up-file-icon">📝</span>
                  <div className="up-doc-info">
                    <div className="up-doc-name">{t.task_title}</div>
                    <div className="up-doc-meta">{t.subject} · Due: {t.deadline}</div>
                  </div>
                  <div className="up-doc-right">
                    <span
                      className="up-status-ok"
                      style={priorityColor(t.ai_analysis?.recommended_priority)}
                    >
                      {t.ai_analysis?.recommended_priority} Priority
                    </span>
                  </div>
                </div>
              ))}
          ...
          <div className="up-how-it-works">
            ...
            <div className="up-how-steps">
              {[
                ...
                { icon: '🧠', label: 'AI identifies tasks & deadlines' },
