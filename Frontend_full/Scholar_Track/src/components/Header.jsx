      ...
      <div className="header__right">
        ...
        <div className="header__profile-wrap">
          ...
          >
            ...
            <svg className={`header__chevron ${profileOpen ? 'open' : ''}`} width="16" height="16" viewBox="0 0 16 16" fill="none">
          ...
          {profileOpen && (
            <div className="header__dropdown">
              <button className="header__drop-item" onClick={() => { navigate('/settings'); setProfileOpen(false); }}>
                <span>👤</span> My Profile
              </button>
              <button className="header__drop-item" onClick={() => { navigate('/notifications'); setProfileOpen(false); }}>
                <span>🔔</span> Notifications
              </button>
              {role === 'admin' && (
                <button className="header__drop-item" onClick={() => { navigate('/system'); setProfileOpen(false); }}>
                  <span>🖥️</span> System Info
                </button>
              )}
              <div className="header__drop-divider" />
              <button className="header__drop-item danger" onClick={() => { setProfileOpen(false); onLogout?.(); }}>
                <span>🚪</span> Sign Out
              </button>
            </div>
          )}
